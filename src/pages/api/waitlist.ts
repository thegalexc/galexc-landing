...
import { getEmailKey, insertWaitlistEntry, normalizeEmail } from '../../lib/waitlist';
...
async function verifyTurnstile(request: Request, token: string | null): Promise<boolean> {
...
export const POST: APIRoute = async ({ request }) => {
  ...
  const ipLimit = await applyRateLimit(`waitlist:ip:${ipAddress ?? 'unknown'}`, 3, 60 * 60);
  if (!ipLimit.allowed) {
    return redirectTo('rate_limited');
  }

  const emailLimit = await applyRateLimit(`waitlist:email:${getEmailKey(normalizedEmail)}`, 3, 60 * 60 * 24);
